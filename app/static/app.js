...
function renderItems() {
  ...
  list.innerHTML=visible.map(item=>`<article class="item-card" data-id="${item.id}"><h3>${escapeHtml(item.title)}</h3><div class="item-meta"><span class="tag">${labels[item.status]||item.status}</span><span class="tag">${labels[item.kind]||item.kind}</span>${item.recurrence?`<span class="tag">${escapeHtml(recurrenceLabel(item.recurrence))}</span>`:item.due_at?`<span class="tag">entro ${displayDate(item.due_at)}</span>`:''}${item.progress_value!=null?`<span class="tag">${item.progress_value}${item.progress_total?`/${item.progress_total}`:''}</span>`:''}</div></article>`).join('');
